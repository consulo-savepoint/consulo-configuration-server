...
class SyncAction extends DumbAwareAction
{
	...
	public void actionPerformed(AnActionEvent e)
	{
		IcsManager.getInstance().sync().doWhenDone(new Runnable()
		{
			...
			public void run()
			{
				new Notification(IcsManager.PLUGIN_NAME, IcsBundle.message("sync.done.title"), IcsBundle.message("sync.done.message"),
		...
		}).doWhenRejected(new Consumer<String>()
		{
			...
			{
				new Notification(IcsManager.PLUGIN_NAME, IcsBundle.message("sync.rejected.title"), IcsBundle.message("sync.rejected.message",
						StringUtil.notNullize(error, "Internal error")), NotificationType.ERROR).notify(null);
			}
		});
	}
}
